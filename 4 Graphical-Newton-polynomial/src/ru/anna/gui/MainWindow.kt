...
import ru.anna.gui.graphics.PainterDerivative
...
class MainWindow : JFrame(){
    ...
    private val derivative = Newton(ArrayList())
    ...
    init{
        ...
        val der = PainterDerivative(plane, derivative)
        ...
        mainPanel.addComponentListener(object : ComponentAdapter() {  // Чтобы обновлялась отрисовка у графика
            override fun componentResized(e: ComponentEvent?) {  // Что делать, если меняется размер панели
                ...
                der.plane.realWidth = mainPanel.width
                der.plane.realHeight = mainPanel.height
            }
        })
        ...
        controlPanel.addValChangeListener{  // Подпишимся на событие изменения значения этих спинеров
            ...
            np.plane.yMin = controlPanel.smYMin.number.toDouble()
            der.plane.xMin = controlPanel.smXMin.number.toDouble()
            der.plane.xMax = controlPanel.smXMax.number.toDouble()
            der.plane.yMin = controlPanel.smYMin.number.toDouble()
            der.plane.yMin = controlPanel.smYMin.number.toDouble()
            mainPanel.repaint()
        }
        ...
        mainPanel.addMouseListener(object : MouseAdapter(){
            override fun mouseClicked(e: MouseEvent?) {
                ...
                if(e!=null){  // Проверяем клик по панели mainPanel
                    if(e.point.x < plane.realWidth && e.point.x > 0 && e.point.y > 0 && e.point.y < plane.realHeight)   // Добавляем в полином точки
                    {
                        derivative.newPoint(Converter.xScr2Crt(e.point.x,plane),Converter.yScr2Crt(e.point.y,plane))
                        mainPanel.repaint()
                    }
                }
            }
        ...
        controlPanel.addColorListener {
            np.setColor(controlPanel.getColor1())
            der.setColor(controlPanel.getColor2())
            mainPanel.paint(mainPanel.graphics)
        }

        mainPanel.addPainter(dp)
        mainPanel.addPainter(np)
        mainPanel.addPainter(der)
