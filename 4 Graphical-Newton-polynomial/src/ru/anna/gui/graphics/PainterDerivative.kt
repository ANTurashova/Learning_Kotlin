...
class PainterDerivative(val plane: CartesianScreenPlane, val newton: Newton) : Painter
{
        ...
        if(g != null){
            ...
            if(newton.xfx.size != 0){
                for(i in 0..n){  // Добавляем точечки
                    pointsArray[plane.xMin + ((plane.xMax-plane.xMin)*i)/n] = newton.derivative().invoke(plane.xMin + ((plane.xMax-plane.xMin)*i)/n)
                }
